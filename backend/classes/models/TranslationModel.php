...
class TranslationModel 
{
    ...
    {
        return $this->db->file('/translate/get_random_pair.sql')
                        ->result(['fetch' => 'assoc'])[0] ?? [];
    }

    public function getPendingTranslation(): array 
    {
        return $this->db->file('/translate/get_one_for_translation.sql')
                        ->result(['fetch' => 'assoc'])[0] ?? [];
    }

    public function saveTranslation(string $translated, int $language_id_2, int $sentence_id_1): void 
    {
        $this->db->file('/sentences/insert_sentence_translation.sql')
